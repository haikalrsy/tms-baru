<template>
  <div class="flex min-h-screen w-full bg-background">
    <aside
      :class="['hidden lg:flex flex-col border-r bg-sidebar transition-[width] duration-300', collapsed ? 'w-[72px]' : 'w-[248px]']"
    >
      ...
      <nav class="flex-1 overflow-y-auto px-3 py-4">
        <ul class="flex flex-col gap-1">
          <template v-for="section in nav.sections" :key="section.label">
            <li v-if="section.label" class="mt-3 first:mt-0">
              <span v-if="!collapsed" class="px-3 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60">
                {{ section.label }}
              </span>
              <div v-else class="mx-3 my-1 border-t border-sidebar-border opacity-40" />
            </li>
        ...
        <aside class="absolute left-0 top-0 h-full w-[260px] bg-sidebar shadow-elevated animate-fade-in">
          ...
          <nav class="px-3 py-4">
            <ul class="flex flex-col gap-1">
              <template v-for="section in nav.sections" :key="section.label">
                <li v-if="section.label" class="mt-3 first:mt-0">
                  <span class="px-3 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60">{{ section.label }}</span>
                </li>
                <li v-for="item in section.items" :key="item.to">
    ...
    <div class="flex min-w-0 flex-1 flex-col">
      <header class="sticky top-0 z-30 flex h-16 items-center gap-3 border-b bg-surface/80 px-4 backdrop-blur-md sm:px-6">
        <button class="rounded-md p-2 hover:bg-muted lg:hidden" @click="mobileOpen = true" aria-label="Open menu">
          <Menu class="h-5 w-5" />
        </button>
        <div class="relative hidden flex-1 max-w-md md:block">
          <Search class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <input placeholder="Search orders, drivers, deliveries..."
        ...
        <button @click="themeStore.toggle()" class="rounded-md p-2 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Toggle theme">
        ...
        <div class="flex items-center gap-3 rounded-lg border bg-surface-2 px-2.5 py-1.5">
          ...
          <button @click="authStore.signOut()" class="inline-flex items-center justify-center rounded-md p-1.5 hover:bg-muted" aria-label="Sign out">
...
import {
  ...
  LogOut, Moon, Sun, Bell, Search, ChevronLeft, Menu, ShoppingCart, Map, ArrowRightLeft,
...
const adminNav = {
  ...
  wms: [
    ...
    { to: "/admin/logistics-map", label: "Logistics Map", icon: Map          },
  ],
}

const driverNav = [
  { to: "/driver",            label: "Dashboard",    icon: LayoutDashboard },
  { to: "/driver/transfers",  label: "My Transfers", icon: ArrowRightLeft  }, 
  { to: "/driver/map",        label: "Live Map",     icon: Map             },
]

const nav = computed(() =>
  props.role === "admin"
    ? { sections: [{ label: "TMS", items: adminNav.tms }, { label: "WMS", items: adminNav.wms }] }
    : { sections: [{ label: "", items: driverNav }] }
)
